...
# --- 2C. GENERADORES DE EXCEL OFICIALES ---
...
with tab_forward:
    ...
    if not st.session_state.tablas_mostrar:
        ...
        # === NUEVA SECCIÓN: GENERADOR PROPHET ===
...
# ==========================================
# PESTAÑA 4: HISTÓRICO VOLA (NUEVA PESTAÑA)
# ==========================================
with tab_vola_hist:
    ...
    if df_vola_reservas is not None:
        ...
        with col_chart:
            df_res_melt = df_vola_reservas.melt(id_vars="Año de reporte", var_name="Moneda", value_name="Tasa")
        ...
        def mostrar_kpis(df, titulo):
            st.markdown(f"**{titulo}**")
            if len(df) >= 2:
                ult = df.iloc[-1]
                ant = df.iloc[-2]
                
                ayuda_pbs = "VOLA de la última fecha registrada. Y debajo está la variación respecto al periodo previo (100 pbs = 1%)."
            ...
            # --- NUEVO: Gráfico de tendencia para la sección GAP ---
            if not df.empty and 'Fecha de reporte' in df.columns:
                df_melt = df.melt(id_vars="Fecha de reporte", var_name="Moneda", value_name="Tasa")
                ...
                fig.update_layout(
                    ...
                    xaxis_title=None,
                    yaxis_title="Tasa (%)",
                    height=250 # Altura compacta para no abrumar la vista
                )
                st.plotly_chart(fig, use_container_width=True)
        ...
        with col_spp:
            mostrar_kpis(df_vola_spp, "VOLA para SPP y SCTR")
            
        with col_vlp:
            mostrar_kpis(df_vola_vlp, "VOLA para Vida Largo Plazo (no SPP/SCTR)")
...
# ==========================================
# PESTAÑA 5: CÁLCULO DE VPN (NUEVA PESTAÑA)
# ==========================================
with tab_vpn:
    st.header("Valor presente neto")
    st.markdown("Calcula el VPN de un flujo de caja mensual descontado con la curva de tasas de descuento anuales (VTDA).")

    col_flujo, col_tasa = st.columns(2)
    
    with col_flujo:
        st.subheader("1. Cargar flujo de caja")
        archivo_flujo = st.file_uploader("Sube el Excel con el flujo (columnas: Periodo, Flujo)", type=["xlsx", "xls"], key="up_flujo")
        
    with col_tasa:
        st.subheader("2. Seleccionar vector de tasas")
        origen_vtd = st.radio("Origen de la VTD:", ["Usar curva calculada en el dashboard", "Cargar vector VTD"])
        
        df_vtd_vpn = None
        if origen_vtd == "Usar curva calculada en el dashboard":
            if st.session_state.tablas_mostrar:
                moneda_vpn = st.selectbox("Moneda generada:", list(st.session_state.tablas_mostrar.keys()))
                df_base = st.session_state.tablas_mostrar[moneda_vpn]
                df_vtd_vpn = pd.DataFrame({"Periodo": df_base["Mes"], "VTDA": df_base["Tasa anual + VOLA (%)"]})
            else:
                st.warning("⚠️ Debes generar primero las curvas en la pestaña 'Análisis General'.")
        else:
            archivo_vtd = st.file_uploader("Sube el Excel con VTD (columnas: Periodo, VTDA)", type=["xlsx", "xls"], key="up_vtd")
            if archivo_vtd:
                try:
                    df_vtd_vpn = pd.read_excel(archivo_vtd)
                    # Normalizar columnas por si tienen otros nombres o la tasa está en formato Excel
                    if 'Periodo' not in df_vtd_vpn.columns or 'VTDA' not in df_vtd_vpn.columns:
                        df_vtd_vpn.columns = ['Periodo', 'VTDA'] + list(df_vtd_vpn.columns[2:])
                    
                    # Convertir porcentajes si es que se detectan mayores a 1
                    if df_vtd_vpn['VTDA'].mean() > 1:
                        df_vtd_vpn['VTDA'] = df_vtd_vpn['VTDA'] / 100
                        
                except Exception as e:
                    st.error(f"Error al leer archivo VTD: {e}")

    if archivo_flujo is not None and df_vtd_vpn is not None:
        st.markdown("---")
        if st.button("🚀 Calcular VPN", type="primary", use_container_width=True):
            try:
                ...
                # Combinar datos de Flujo con VTD
                df_calc = pd.merge(df_flujo[['Periodo', 'Flujo']], df_vtd_vpn[['Periodo', 'VTDA']], on='Periodo', how='left')
                
                # Extrapolación: Si el flujo es más largo que la VTD, se repite la última tasa conocida
                if df_calc['VTDA'].isna().any():
                    df_calc['VTDA'] = df_calc['VTDA'].ffill()
                    st.toast("💡 El flujo excede los periodos de la VTD. Se ha extrapolado de forma plana la última tasa.")
                
                # Manejo de tasa en el periodo 0 (si la VTD empezara en el mes 1, arrastramos hacia atrás)
                if df_calc['VTDA'].isna().any():
                    df_calc['VTDA'] = df_calc['VTDA'].bfill()

                # Fórmula matemática de descuento Spot Anual: VPN = Flujo / (1 + S_t)^(t/12)
                ...
                st.markdown(
                    f"""
                    <div style="background-color: #EAF0F8; padding: 15px; border-radius: 5px; border-left: 6px solid #06369D;">
                        <h3 style="color: #06369D; margin: 0;">💰 Valor Presente Neto (VPN): {vpn_total:,.2f}</h3>
                    </div>
                    """, 
                ...
                with pd.ExcelWriter(buffer_vpn, engine='openpyxl') as writer:
                    df_calc.to_excel(writer, index=False, sheet_name='Calculo_VPN')
                
                st.download_button(
                    label="📥 Descargar Detalle del VPN en Excel", 
                    data=buffer_vpn.getvalue(), 
                    file_name="Detalle_VPN_Flujos.xlsx", 
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                )
