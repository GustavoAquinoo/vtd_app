...
st.set_page_config(page_title="Interfaz de Vector de Tasas (VTD)", layout="wide")
...
# --- Sincronización de Periodos y VOLA entre Comparativa y VPN ---
sync_defaults = {
    'p1_ini': datetime.date(2024, 1, 1), 'p1_fin': datetime.date(2024, 12, 31), 'vola_p1': 0.1502,
    'p2_act': True, 'p2_ini': datetime.date(2025, 1, 1), 'p2_fin': datetime.date(2025, 12, 31), 'vola_p2': 0.1502,
    'p3_act': False, 'p3_ini': datetime.date(2025, 6, 1), 'p3_fin': datetime.date(2025, 12, 31), 'vola_p3': 0.1502
}

# Inyectar en session_state para ambas pestañas
for k, val in sync_defaults.items():
    if f"comp_{k}" not in st.session_state: st.session_state[f"comp_{k}"] = val
    if f"vpn_{k}" not in st.session_state: st.session_state[f"vpn_{k}"] = val

# Callbacks para actualizar la otra pestaña instantáneamente
def sync_to_vpn():
    for k in sync_defaults.keys(): st.session_state[f"vpn_{k}"] = st.session_state[f"comp_{k}"]

def sync_to_comp():
    for k in sync_defaults.keys(): st.session_state[f"comp_{k}"] = st.session_state[f"vpn_{k}"]
# ------------------------------------------------------------------

# --- 2A. CARGA EN MEMORIA ---
@st.cache_data
def cargar_datos_en_memoria(ruta_archivo, mtime):
    """Lee el archivo crudo y lo deja pivoteado y listo en RAM.
    'mtime' (fecha de modificación del archivo) hace que el caché se renueve
    automáticamente cuando se reemplaza el Excel con datos nuevos."""
    try:
...
for moneda, ruta in archivos_fuente.items():
    if os.path.exists(ruta):
        bases_de_datos[moneda] = cargar_datos_en_memoria(ruta, os.path.getmtime(ruta))

@st.cache_data
def cargar_vola_historico(ruta, mtime):
    """Lee las 3 hojas del histórico VOLA abriendo el archivo una sola vez.
    'mtime' renueva el caché cuando se actualiza el archivo."""
    hojas = pd.read_excel(
        ruta,
        sheet_name=["VOLA (no SPP-SCTR)", "VOLA SPP-SCTR", "VOLA VLP"],
        skiprows=2
    )
    return [df.loc[:, ~df.columns.str.contains('^Unnamed')] for df in hojas.values()]
...
# --- Funciones de variación porcentual de VPN entre escenarios ---
def _var_pct(vpn_inicial, vpn_final):
    """(final - inicial) / |inicial|. Devuelve NaN si el valor inicial es 0."""
    if vpn_inicial == 0 or pd.isna(vpn_inicial):
        return np.nan
    return (vpn_final - vpn_inicial) / abs(vpn_inicial)

def calcular_tabla_variacion_vpn(df_res, base_label):
    """Compara cada escenario contra el escenario base elegido."""
    df = df_res.copy()
    df["Nombre"] = df["Escenario"].str.replace("\n", " ", regex=False)
    df["Corto"] = df["Escenario"].str.split("\n").str[0]
    fila_base = df[df["Nombre"] == base_label].iloc[0]
    vpn_base = fila_base["VPN Total"]

    filas = []
    for _, r in df[df["Nombre"] != base_label].iterrows():
        var = _var_pct(vpn_base, r["VPN Total"])
        if pd.isna(var):
            lectura = "No calculable (VPN base = 0)"
        elif var == 0:
            lectura = f"{r['Corto']} tiene el mismo VPN que {fila_base['Corto']}"
        else:
            sentido = "mayor" if var > 0 else "menor"
            lectura = f"El VPN de {r['Corto']} es {abs(var):.2%} {sentido} que el de {fila_base['Corto']}"
        filas.append({
            "Comparación": f"{r['Corto']} vs {fila_base['Corto']}",
            "VPN base (inicial)": vpn_base,
            "VPN comparado (final)": r["VPN Total"],
            "Diferencia (final − inicial)": r["VPN Total"] - vpn_base,
            "Var. %": var,
            "Lectura": lectura,
        })
...
with tab_comparativa:
    ...
    with col1:
        ...
        p1_inicio = st.date_input("Inicio P1", min_value=datetime.date(2015, 1, 1), max_value=datetime.date(2030, 12, 31), key="comp_p1_ini", on_change=sync_to_vpn)
        p1_fin = st.date_input("Fin P1", min_value=datetime.date(2015, 1, 1), max_value=datetime.date(2030, 12, 31), key="comp_p1_fin", on_change=sync_to_vpn)
        p1_vola = st.number_input("VOLA P1 (%)", format="%.4f", key="comp_vola_p1", on_change=sync_to_vpn) / 100
        periodos_config.append(("Periodo 1", p1_inicio, p1_fin, p1_vola))

    with col2:
        st.markdown("##### 📅 Periodo 2")
        p2_act = st.checkbox("Habilitar Periodo 2", key="comp_p2_act", on_change=sync_to_vpn)
        p2_inicio = st.date_input("Inicio P2", min_value=datetime.date(2015, 1, 1), max_value=datetime.date(2030, 12, 31), key="comp_p2_ini", on_change=sync_to_vpn)
        p2_fin = st.date_input("Fin P2", min_value=datetime.date(2015, 1, 1), max_value=datetime.date(2030, 12, 31), key="comp_p2_fin", on_change=sync_to_vpn)
        p2_vola = st.number_input("VOLA P2 (%)", format="%.4f", key="comp_vola_p2", on_change=sync_to_vpn) / 100
        if p2_act:
            periodos_config.append(("Periodo 2", p2_inicio, p2_fin, p2_vola))

    with col3:
        st.markdown("##### 📅 Periodo 3")
        p3_act = st.checkbox("Habilitar Periodo 3", key="comp_p3_act", on_change=sync_to_vpn)
        p3_inicio = st.date_input("Inicio P3", min_value=datetime.date(2015, 1, 1), max_value=datetime.date(2030, 12, 31), key="comp_p3_ini", on_change=sync_to_vpn)
        p3_fin = st.date_input("Fin P3", min_value=datetime.date(2015, 1, 1), max_value=datetime.date(2030, 12, 31), key="comp_p3_fin", on_change=sync_to_vpn)
        p3_vola = st.number_input("VOLA P3 (%)", format="%.4f", key="comp_vola_p3", on_change=sync_to_vpn) / 100
        if p3_act:
...
with tab_vola_hist:
    ...
    try:
        if os.path.exists(archivo_vola):
            df_vola_reservas, df_vola_spp, df_vola_vlp = cargar_vola_historico(
                archivo_vola, os.path.getmtime(archivo_vola)
            )
    ...
    if df_vola_reservas is not None:
        ...
        def mostrar_kpis(df, titulo, mostrar_todo=False):
            ...
            if not df.empty and 'Fecha de reporte' in df.columns:
                ...
                st.plotly_chart(fig, use_container_width=True, key=f"graf_vola_{titulo}")
...
with tab_vpn:
    ...
    if origen_vtd == "Generar escenarios múltiples":
        ...
        with col_p1:
            ...
            e1_inicio = st.date_input("Inicio E1", key="vpn_p1_ini", on_change=sync_to_comp)
            e1_fin = st.date_input("Fin E1", key="vpn_p1_fin", on_change=sync_to_comp)
            e1_vola = st.number_input("VOLA E1 (%)", format="%.4f", key="vpn_vola_p1", on_change=sync_to_comp) / 100
            escenarios_vpn.append(("Escenario 1", e1_inicio, e1_fin, e1_vola))
 
        with col_p2:
            st.markdown("##### 📅 Escenario 2")
            e2_act = st.checkbox("Habilitar Escenario 2", key="vpn_p2_act", on_change=sync_to_comp)
            e2_inicio = st.date_input("Inicio E2", key="vpn_p2_ini", on_change=sync_to_comp)
            e2_fin = st.date_input("Fin E2", key="vpn_p2_fin", on_change=sync_to_comp)
            e2_vola = st.number_input("VOLA E2 (%)", format="%.4f", key="vpn_vola_p2", on_change=sync_to_comp) / 100
            if e2_act:
                escenarios_vpn.append(("Escenario 2", e2_inicio, e2_fin, e2_vola))
 
        with col_p3:
            st.markdown("##### 📅 Escenario 3")
            e3_act = st.checkbox("Habilitar Escenario 3", key="vpn_p3_act", on_change=sync_to_comp)
            e3_inicio = st.date_input("Inicio E3", key="vpn_p3_ini", on_change=sync_to_comp)
            e3_fin = st.date_input("Fin E3", key="vpn_p3_fin", on_change=sync_to_comp)
            e3_vola = st.number_input("VOLA E3 (%)", format="%.4f", key="vpn_vola_p3", on_change=sync_to_comp) / 100
            if e3_act:
                escenarios_vpn.append(("Escenario 3", e3_inicio, e3_fin, e3_vola))
 
    else:
        st.subheader("3. Cargar archivos VTD por caso")
        st.caption("Cada caso es una curva VTD distinta (columnas: Periodo, VTDA). Puedes renombrarlo para identificarlo mejor en los resultados.")
        col_m1, col_m2, col_m3 = st.columns(3)
        archivos_manuales = []
        nombres_usados = set()

        def _nombre_caso(n, key):
            """Devuelve el nombre ingresado o 'Caso n' si está vacío o repetido."""
            nombre = st.text_input(f"Nombre del caso {n}", value=f"Caso {n}", key=key).strip()
            if not nombre or nombre in nombres_usados:
                nombre = f"Caso {n}"
        ...
        with col_m1:
            st.markdown("##### 📂 Caso 1")
            nombre_c1 = _nombre_caso(1, "nom_caso1")
            archivo_vtd1 = st.file_uploader("VTD del caso 1 (Periodo, VTDA)", type=["xlsx", "xls"], key="up_vtd1")
            if archivo_vtd1:
                archivos_manuales.append((nombre_c1, archivo_vtd1))

        with col_m2:
            st.markdown("##### 📂 Caso 2")
            vtd2_act = st.checkbox("Habilitar Caso 2", value=False, key="vtd2_act")
            if vtd2_act:
                nombre_c2 = _nombre_caso(2, "nom_caso2")
                archivo_vtd2 = st.file_uploader("VTD del caso 2 (Periodo, VTDA)", type=["xlsx", "xls"], key="up_vtd2")
                if archivo_vtd2:
                    archivos_manuales.append((nombre_c2, archivo_vtd2))

        with col_m3:
            st.markdown("##### 📂 Caso 3")
            vtd3_act = st.checkbox("Habilitar Caso 3", value=False, key="vtd3_act")
            if vtd3_act:
                nombre_c3 = _nombre_caso(3, "nom_caso3")
                archivo_vtd3 = st.file_uploader("VTD del caso 3 (Periodo, VTDA)", type=["xlsx", "xls"], key="up_vtd3")
                if archivo_vtd3:
                    archivos_manuales.append((nombre_c3, archivo_vtd3))
            ...
            try:
                ...
                if origen_vtd == "Generar escenarios múltiples":
                    ...
                    if archivos_manuales:
                        for etiqueta, archivo in archivos_manuales:
                            try:
                                df_vtd_manual = pd.read_excel(archivo)
                                ...
                                st.error(f"Error al procesar el archivo de {etiqueta}: {e}")
                    else:
                        st.warning("⚠️ Debes cargar al menos un archivo Excel válido con la VTD para calcular.")
 
                if extrapolado_flag:
                    st.toast("💡 El flujo excede los periodos de la VTD. Se ha extrapolado de forma plana la última tasa.")

                # Guardamos el resultado en memoria para que la sección de variaciones no desaparezca al interactuar con ella.
                if resultados_vpn:
                    st.session_state.vpn_calc = {
                        "df_res": pd.DataFrame(resultados_vpn),
                        "df_det": df_detalles,
                    }
                else:
                    st.session_state.pop("vpn_calc", None)

            except Exception as e:
                st.session_state.pop("vpn_calc", None)
                st.error(f"Error en el cálculo: Verifica la estructura de tus archivos. Detalle: {e}")

        # --- Renderizado de Resultados (persistente entre interacciones) ---
        if st.session_state.get("vpn_calc"):
            df_res_vpn = st.session_state.vpn_calc["df_res"]
            df_detalles = st.session_state.vpn_calc["df_det"]
            resultados_vpn = df_res_vpn.to_dict("records")

            st.subheader("📊 Resultados de VPN")
            st.caption("Resultados del último cálculo. Si cambias parámetros, vuelve a presionar **Calcular VPN**.")
            ...
            fig_bar.update_layout(
                ...
                yaxis=dict(title="", tickformat=',.2f', showgrid=False, zeroline=False, range=rango_y),
            ...
            df_var_tabla = None
            st.markdown("---")
            st.subheader("📐 Variación porcentual entre escenarios")

            if len(df_res_vpn) < 2:
                st.info("Habilita al menos 2 escenarios (o 2 casos) para ver la variación porcentual entre sus VPN.")
            else:
                nombres = df_res_vpn["Escenario"].str.replace("\n", " ", regex=False).tolist()

                col_base, col_formula = st.columns([1, 2])
                with col_base:
                    base_sel = st.selectbox(
                        "Escenario base (valor inicial):",
                        nombres,
                        index=0,
                        key="vpn_base_var",
                        help="Todos los demás escenarios se comparan contra este VPN."
                    )
                with col_formula:
                    st.latex(r"\text{Var. \%} = \frac{VPN_{\text{comparado (final)}} - VPN_{\text{base (inicial)}}}{\left|VPN_{\text{base (inicial)}}\right|} \times 100")
                    st.caption(
                        "Positivo (▲): el escenario comparado tiene **mayor** VPN que la base. "
                        "Negativo (▼): tiene **menor** VPN. Se usa el valor absoluto del VPN base "
                        "para que el signo siga siendo correcto aun si el VPN base es negativo."
                    )

                df_var_tabla = calcular_tabla_variacion_vpn(df_res_vpn, base_sel)

                def _color_var(col):
                    return ["color: #1B7F3B; font-weight: 600" if pd.notna(v) and v > 0
                            else "color: #C0392B; font-weight: 600" if pd.notna(v) and v < 0
                            else "" for v in col]
                ...
                st.dataframe(
                    df_var_tabla.style
                        .format({
                            "VPN base (inicial)": "{:,.2f}",
                            "VPN comparado (final)": "{:,.2f}",
                            "Diferencia (final − inicial)": "{:+,.2f}",
                            "Var. %": "{:+.4%}",
                        }, na_rep="n.d.")
                        .apply(_color_var, subset=["Var. %"]),
                    use_container_width=True,
                    hide_index=True
                )
            ...
            with pd.ExcelWriter(buffer_vpn, engine='openpyxl') as writer:
                ...
                if df_var_tabla is not None:
                    df_var_tabla.to_excel(writer, index=False, sheet_name='Variacion_VPN')
